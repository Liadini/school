...
public class Rainbow extends JPanel
{
  ...
  public void paintComponent(Graphics g) //draws whatever is specified in the curly brackets
  {
	...
	g.fillArc(xCenter - largeRadius, yCenter - largeRadius, largeRadius * 2, largeRadius * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is largeRadius*2 by largeRadius*2 and starts at (xCenter-largeRadius, yCenter-largeRadius)

	g.setColor(Color.ORANGE); //color is now orange
	g.fillArc(xCenter - radius2, yCenter - radius2, radius2 * 2, radius2 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius2*2 by radius2*2 and starts at (xCenter-radius2, yCenter-radius2)
	
	g.setColor(Color.YELLOW); //color is now yellow
	g.fillArc(xCenter - radius3,  yCenter - radius3, radius3 * 2, radius3 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius3*2 by radius3*2 and starts at (xCenter-radius3, yCenter-radius3)
	
	g.setColor(Color.GREEN); //color is now green
	g.fillArc(xCenter - radius4,  yCenter - radius4, radius4 * 2, radius4 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius4*2 by radius4*2 and starts at (xCenter-radius4, yCenter-radius4)
	
	g.setColor(Color.BLUE); //color is now blue
	g.fillArc(xCenter - radius5,  yCenter - radius5, radius5 * 2, radius5 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius5*2 by radius5*2 and starts at (xCenter-radius5, yCenter-radius5)
	
	g.setColor(Color.MAGENTA); //color is now magenta
	g.fillArc(xCenter - radius6,  yCenter - radius6, radius6 * 2, radius6 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius6*2 by radius6*2 and starts at (xCenter-radius6, yCenter-radius6)
	
	g.setColor(Color.CYAN); //color is now cyan
	g.fillArc(xCenter - radius7,  yCenter - radius7, radius7 * 2, radius7 * 2, 0, 180); //fills 180 degree arc that is inscribed in a rectangle that is radius7*2 by radius7*2 and starts at (xCenter-radius7, yCenter-radius7)
  }
