...
public class PollDisplayPanel extends JPanel {
	...
	private void drawPieChart(Graphics g, int x, int y, int r) {
		...
		// Find the largest count (and identify ties)
		if (count1 == count2 && count1 == count3 && total > 0) {
			tie = (new ImageIcon("tie.png")).getImage();
			g.drawImage(tie, 10, 10, 100, 100, this);
		}
		if (count1 > count2 && count1 > count3) {
			donuts = (new ImageIcon("donuts.png")).getImage();
			g.drawImage(donuts, 10, 10, 149, 98, this);
		}

		if (count2 > count1 && count2 > count3) {
			cake = (new ImageIcon("cake.png")).getImage();
			g.drawImage(cake, 10, 10, 135, 119, this);
		}

		if (count3 > count1 && count3 > count2) {
			pie = (new ImageIcon("pie.png")).getImage();
			g.drawImage(pie, 10, 10, 149, 98, this);
		}
	}
